...
<div class="contain" ondblclick="chiudi(this)">
    <div class="contain_all">
        ...
        <!-- UNICO FORM che gestisce ordinamento, categoria e ricerca -->
            ...
            <div class="filtri-ordine">
                ...
                <div class="ordine-group">
                    ...
                    <select name="ord" id="ordineSelect" onchange="this.form.submit()">
                        ...
                        <option value="PREZZO_DESC" ${ord == 'PREZZO_DESC' ? 'selected' : ''}>Prezzo: dal più alto</option>
        ...
        <c:choose>
            <c:when test="${prodotti.size() > 0}">
                ...
                <div class="paginazione">
                    <c:if test="${npag > 1}">
                        <c:if test="${pag > 1}">
                            <a href="?categoria=${categoriaId}&ord=${ord}&q=${param.q}&pag=${pag - 1}&perpag=${perpag}">&larr; precedente</a>
                        </c:if>
                        &emsp;
                        <c:forEach begin="1" end="${npag}" var="i">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a href="?categoria=${categoriaId}&ord=${ord}&q=${param.q}&pag=${i}&perpag=${perpag}">${i}</a>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                        &emsp;
                        <c:if test="${pag < npag}">
                            <a href="?categoria=${categoriaId}&ord=${ord}&q=${param.q}&pag=${pag + 1}&perpag=${perpag}">successiva &rarr;</a>
                        </c:if>
                    </c:if>
                </div>
            ...
            <c:otherwise>
                ...
                <p>Prova a cambiare filtro o categoria, oppure modifica la ricerca.</p>
            </c:otherwise>
        </c:choose>
...
<!-- JavaScript per l'autocompletamento AJAX (senza reindirizzamento automatico) -->
<script>
